...
module.exports = {
  ...
  async fn(inputs) {
    ...
    const [isFavorite, projectManagers, backgroundImages, baseCustomFieldGroups] =
      await Promise.all([
        sails.helpers.users.isProjectFavorite(currentUser.id, project.id),
        ProjectManager.qm.getByProjectId(project.id),
        BackgroundImage.qm.getByProjectId(project.id),
        BaseCustomFieldGroup.qm.getByProjectId(project.id),
      ]);
    ...
    const [users, customFields] = await Promise.all([
      ...
      CustomField.qm.getByBaseCustomFieldGroupIds(baseCustomFieldGroupsIds),
    ]);
